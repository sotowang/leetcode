...
public class SuperPow {
    ...
    private int pow(int a,int[] b,int index){
        if(index == -1){
            return 1;
        }
        int count = b[index];
        int first = myPow(a,count,1337);
        int third = myPow(pow(a,b,index-1),10,1337);
        return ( first*third)%1337;
    }
    private int myPow(int a,int b,int mod){
        if(b==0){
            return 1;
        }
        a %= mod;
        if((b & 1) == 0){
            int half = myPow(a,b>>1,mod);
